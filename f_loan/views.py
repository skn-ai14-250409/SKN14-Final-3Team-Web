...
def check_customer(request):
    ...
    try:
        ...
        customer_data = {
            ...
            'education_level_name': person.education_level.name if person.education_level else '정보 없음',
            'company_name': person.company_name or '정보 없음',
            ...
            'housing_status_name': person.housing_status.name if person.housing_status else '정보 없음',
    ...
    try:
        ...
        try:
            ...
            if not corp:
                ...
                # legal_name(대표자명)이 아닌 company_name(기업명)으로 검색
                corp = next((c for c in all_corps if clean_company_name(c.company_name) == clean_legal_name), None)

            if not corp:
                raise CustomerCorporate.DoesNotExist

            customer = corp.customer
            logger.info(f"✅ [DB] 기업 고객 정보 발견: {corp.company_name} (ID: {customer.seq_id})")

            customer_data = {
                'id': customer.seq_id,
                'full_name': corp.company_name,
...
def generate_recommendation(prediction_result, customer_data, loan_data):
    ...
    if approval_status == 'approved':
        ...
        if customer_data.get('housing_status_name') != '자가':
            warnings.append(f"주택 상태({customer_data.get('housing_status_name')}) 고려, 안정성 확보 필요")
